...
const index = (req,res,next)=>{
    ...
    Campus.find()
.then(response =>{
    var data =  {campus :response,user: req.session.user.user}; 
                                    res.render('faculte', data)
})
.catch(error =>{
    res.json({
...
const update =(req,res,next) =>{
    ...
    if(req.files){
        ...
        path =path.substring(0,path.lastIndexOf(","))
        updateData.image = path.length==0?req.body.replaceImage:path;
    }
...
.catch(error =>{
 ...
 function deleteimage(req){
  ...
  req.body.path.split(',').forEach(path=>{
        fs.unlink(path, (err) => {
            if (err) {
            console.error(err)
...
module.exports={
    index,destroy,show,store,update,getallplace,get
}